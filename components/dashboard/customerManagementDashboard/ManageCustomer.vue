...
// pinia
const { customerLists } = storeToRefs(useCustomerOrderManageDashboardStore());

const { verifyCustomer, deleteCustomer } =
      ...
      <div class="-mx-4 sm:-mx-8 px-4 sm:px-8 py-4 overflow-x-auto">
        <div class="inline-block min-w-full shadow rounded-lg overflow-hidden">
          <table class="min-w-full leading-normal">
            ...
            <tbody>
              <tr v-for="singleOrder in customerLists">
                ...
                <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                  <p class="text-gray-900 whitespace-no-wrap">
                    {{ singleOrder.customerId }}
                  </p>
                </td>
                <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                  <p class="text-gray-900 whitespace-no-wrap">
                    {{ singleOrder.totalPurchase }}
                  </p>
                </td>
                <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                  <p class="text-gray-900 whitespace-no-wrap">
                    {{ singleOrder.purchaseTime }}
                  </p>
                </td>
                <td
                ...
                >
                  <span
                    class="relative font-semibold text-green-600 w-10 h-10 flex justify-center items-center rounded-full leading-tight cursor-pointer"
                    @click="handleVerifyCustomer(singleOrder.id)"
                  >
                    <BaseIconVerifiedIcon
                      :class="`w-full h-full ${
                        singleOrder.isVerified ? 'text-blue-700' : ''
                      }`"
                    />
                  </span>
                  <span
                    ...
                    @click="handleDeleteCustomer(singleOrder.id)"
                  >
                    <fa
