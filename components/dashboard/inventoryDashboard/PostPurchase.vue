...
// pinia
const { purchaseProductsList } = storeToRefs(useInventoryDashboardStore());
const { approvePurchaseList, deletePurchaseList } =
    ...
    <div class="-mx-4 sm:-mx-8 px-4 sm:px-8 py-4 overflow-x-auto">
      <div class="inline-block min-w-full shadow rounded-lg overflow-hidden">
        <table class="min-w-full leading-normal">
          ...
          <tbody>
            <template v-for="singleProduct in purchaseProductsList">
              <tr v-if="!singleProduct.isApproved">
                ...
                <td
                  class="px-5 py-5 border-b border-gray-200 bg-white text-sm flex items-center"
                >
                  <BaseButton
                    :buttonText="`Approve`"
                    :extraClass="`w-40 rounded-md text-base`"
                    @click="approvePurchaseList(singleProduct.id)"
                  />

                  <span
                    class="text-red-500 px-2 cursor-pointer"
                    @click="deletePurchaseList(singleProduct.id)"
                  >
                    <fa
