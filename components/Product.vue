...
<template>
  ...
  <div class="rounded-xl bg-white drop-shadow-md mb-2 md:w-[200px]">
    ...
    <div class="p-2 flex flex-col justify-between">
      ...
      <p class="text-center text-lg font-bold">$ {{ props.product.price }}</p>

      <div class="flex justify-center items-center py-2 font-bold gap-1">
        <Star class="w-3 h-3" v-for="rating in 5" v-if="!props.product.rating" />
        <svg
          v-else
          v-for="rating in Math.floor(props.product.rating)"
        ...
        <Star
          ...
          v-for="star in 5 - Math.floor(props.product.rating)"
        />
      </div>
      <div
        v-if="isInCart > 0"
      ...
      >
        <button
          v-if="isInCart > 1"
          @click="store.decresecart(props.product.id)"
        ...
        <button
          v-else
          @click="removeCart(props.product.id)"
          class="px-2 text-white border-r font-normal flex items-center justify-center"
        >
          <Minus />
          <ion-icon name="remove-outline" class="text-white"></ion-icon>
        ...
        <button
          @click="store.increaseCart(props.product.id)"
