...
<template>
  <section>
    <div
      class="container-fluid background min-h-screen py-4 flex items-center justify-center"
    >
      <div class="w-full md:w-4/5 h-full row">
        <div v-if="error && error.statusCode === 404" class="w-full mx-auto">
          <svg
            width="100%"
            height="100%"
            viewBox="0 0 636 324"
        ...
        <div class="w-full text-center mt-8">
          <p class="text-white text-2xl lg:text-3xl mb-5">
            {{ error.message }}
          </p>
          <NuxtLink
            to="/"
            class="bg-[#1a1139] w-52 mx-auto text-white font-medium text-lg py-2 px-10 rounded-full text-center"
          >
            Go to Home
          </NuxtLink>
        </div>
...
.background {
  ...
  background: radial-gradient(at 50% -20%, #908392, #1a1139) fixed;
}
